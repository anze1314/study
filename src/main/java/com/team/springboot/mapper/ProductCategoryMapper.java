...
public interface ProductCategoryMapper {
    ...
    @Select("select p_Id,p_Account,p_Name,c_Name,p_Title,p_Price,p_href from product inner join category on product.c_Id=category.c_Id where product.p_state = 0 limit #{0}, #{1}")
    ...
    @Select("select p_Id,p_Account,p_Name,c_Name,p_Title,p_Price,p_href from product inner join category on product.c_Id=category.c_Id where product.p_state = 0 ")
    List<ProductCategory> selectProductAll();

    @Select("select p_href, p_Id,p_Account,p_Name,c_Name,p_Title,p_Price from product inner join category on product.c_Id=category.c_Id where product.p_state = 0 and (p_Title like #{2} OR p_Name like #{2}) limit #{0}, #{1}")
    List<ProductCategory>selectProductCategorysByp_name(int page, int limit,String p_Name);

    @Select("select p_href, p_Id,p_Account,p_Name,c_Name,p_Title,p_Price from product inner join category on product.c_Id=category.c_Id where product.p_state = 0 and (p_Title like #{2} OR p_Name like #{2}) AND p_Account = #{3} limit #{0}, #{1}")
    ...
    @Select("select p_href, p_Id,p_Account,p_Name,c_Name,p_Title,p_Price from product inner join category on product.c_Id=category.c_Id where product.p_state = 0 and (p_Title like #{0} OR p_Name like #{0})")
    List<ProductCategory>selectProductCategorysByp_name1(String p_Name);

    @Select("select p_href, p_Id,p_Account,p_Name,c_Name,p_Title,p_Price from product inner join category on product.c_Id=category.c_Id   ORDER BY RAND() ")
